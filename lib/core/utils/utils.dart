...
class Utils {
  ...
  static void logWithJson(
    ...
    try {
      final prettyprint = const JsonEncoder.withIndent(
        '  ',
  ...
  static String convertDateTimeToPatternTime(
    DateTime? now, {
  ...
  static List<T>? jsonToList<T>(
    ...
    try {
      return isNullOrEmpty(json)
          ? []
          : (json as List)
                .map((model) => factory(model as Map<String, dynamic>))
                .toList();
    } on Exception catch (_) {
      return null;
    }
