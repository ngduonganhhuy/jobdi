...
import 'package:jobdi/core/services/api_service/interceptors/debug_interceptor.dart';
import 'package:jobdi/core/services/api_service/interceptors/encoding_params_interceptor.dart';
import 'package:jobdi/core/services/api_service/interceptors/refresh_interceptor.dart';
import 'package:jobdi/core/utils/logger.dart';
import 'package:jobdi/core/utils/logout_helper.dart';
import 'package:jobdi/core/utils/path_helper.dart';
...
class ApiService {
  ...
  static final Dio refreshTokenDio = Dio(BaseOptions(baseUrl: Env.baseUrl))
    ..interceptors.add(EncodingParamsInterceptor())
    ..interceptors.add(
      kDebugMode ? DebugInterceptor(printOnSuccess: true) : const Interceptor(),
    );
  ...
  static Future<dynamic> callRequest(
    ...
    try {
      ...
      final options = Options(
        ...
        extra: {
          'name': apiDescription,
          'request_time': Utils.convertDateTimeToPatternTime(
            DateTime.now(),
            pattern: 'hh:mm:ss a',
          ),
      ...
      await _handelDioError(url, error);
    } on Exception catch (_) {
      throw ApiException(error: ApiErrorMessage.APP_API_ERROR);
    }
  }

  static Future<void> _handleStatusCodeValidate(
    int? statusCode,
    String? errorMessage,
    int? errorCode,
  ) async {
  ...
  static Future<void> _handelDioError(String url, DioException error) async {
    ...
    if (dioResponse != null) {
      ...
      if (dioResponse.data is Map<String, dynamic>) {
        ...
        final errorResponse = Utils.jsonToModel<ApiException>(
          data,
        ...
        try {
          ...
          errorMessage =
              errorResponse?.error ??
              '${ApiErrorMessage.ERROR_HAPPENS} (code: ${dioResponse.statusCode})';
        } on Exception catch (_) {
          errorMessage = ApiErrorMessage.APP_PARSE_ERROR;
          errorCode = ApiStatusCode.AppCastError;
        }
      }
      await _handleStatusCodeValidate(statusCode, errorMessage, errorCode);
    } else {
